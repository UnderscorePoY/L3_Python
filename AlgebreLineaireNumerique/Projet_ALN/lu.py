...
# [1 bis] METHODES SANS PERMUTATION TRIDIAGONALES
def _factorisation_LU_tridiagonal(
        A: list,
) -> list:
    """
    Factorise la matrice carrée `A` suivant la décomposition LU dans le cas où `A` est tridiagonale:
      - les coefficients sous-diagonaux correspondent à la matrice L (Lower)
      à laquelle doit être ajoutée une diagonale de 1.
      - les coefficient diagonaux et sur-diagonaux correspondent à la matrice U (Upper).
    Ainsi A = LU, et la résolution de Ax = b se décompose comme suit :
    ...
    #      j=0         n-2
    # i=0 (a11 a12  0  ...  0 ) => (a11     a12     ...  0 )
    #     (a21 a22 a23 ...  0 ) => (a21/a11 a22     ...  0 )
    #     ( 0  a32 a33 ...  0 ) => (   0    a32/a22 ...  0 )
    #     (... ... ... ... ...) => (...     ...     ... ...)
    # n-1 ( 0   0  ... ... ann) => (   0       0    ... ann)
    for j in range(n - 1):
        A[j+1][j] /= A[j+1][j+1]
...
def factorisation_LU_tridiagonal(
        A: list,
        keep: bool = False
) -> list:
    """
    Factorise la matrice carrée `A` suivant la décomposition LU tridiagonal.
    :param A: la matrice à factoriser.
    :param keep: (default=False) si la matrice doit être gardée intacte. Sinon, les coefficients de L et U seront
    écrits dans la matrice `A` initiale  pour économiser de l'espace mémoire.
    :return: la matrice `A` factorisée.
    ...
    return _factorisation_LU_tridiagonal(B)


def _descente_tridiagonal(
        A: list,
        b: list
) -> list:
    """
...
def descente_tridiagonal(
    ...
    return _descente_tridiagonal(A, y)


def remontee_tridiagonal(
        A: list,
        y: list
) -> list:
    """
...
'''
# Test tridiagonal
M = [[8, 7, 0, 0], [16, 20, 5, 0], [0, 18, 19, 3], [0, 0, 16, 14]]
LU = factorisation_LU_tridiagonal(M, keep=True)
print(LU)  # LU attendue = [[8, 7, 0, 0], [2.0, 6.0, 5, 0], [0, 3.0, 4.0, 3], [0, 0, 4.0, 2.0]]

B = [6, 9, -33, -104]
Y = descente_tridiagonal(LU, B, keep=True)
print(Y)  # Y attendu = [6, -3, -24, -8]

X = remontee_tridiagonal(LU, Y)
print(X)  # X attendu = [-1, 2, -3, -4]
'''
